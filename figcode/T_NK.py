...
FIG_NAME = "TNK"
H5AD_PATH = PROJECT_DIR / "h5ad" / "T_NK.h5ad"

CELLTYPE_KEY = "celltype"
UMAP_KEYS = ["celltype", "group", "sample"]
UMAP_PANEL_KEYS = ["celltype", "sample", "group"]
...
marker_genes_dict = {
    "FGFBP2+ cytotoxic NK": [
        "NKG7", "FGFBP2", "GNLY", "GZMB", "KLRD1"
    ],

    "FOXP3+ Treg": [
        "FOXP3", "IL2RA", "CTLA4", "TNFRSF18", "TIGIT"
    ],

    "IL7R+ CD4 T": [
        "IL7R", "LTB", "GPR183", "CXCR4", "TCF7"
    ],

    "GZMK+ CD8 T": [
        "CD8A", "CD8B", "CCL5", "GZMK", "ZNF683"
    ],

    "XCL1+ innate-like T/NK": [
        "XCL1", "XCL2", "TRDC", "IL2RB", "KLRC1"
    ],

    "FOXP1+ memory T": [
        "FOXP1", "FYN", "SKAP1", "THEMIS", "TOX"
    ],
}
...
def plot_umaps(adata):
    panel_keys = [
        key for key in UMAP_PANEL_KEYS
        if key in adata.obs or key in adata.var_names
    ]
    if panel_keys:
        axes = ph.plot_umap_panel(
            adata,
            color=panel_keys,
            save=UMAP_DIR / f"{FIG_NAME}_umap_celltype_sample_group.pdf",
        )
        close_scanpy_axes(axes)
